...
export default function AdminSidebar({ isCollapsed, setIsCollapsed }) {
  ...
  return (
    <div
      className={`${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} shadow-xl shrink-0 fixed left-0 top-0 bottom-0 flex flex-col border-r z-40 overflow-hidden ${
        isCollapsed ? "w-[72px]" : "w-[240px]"
      }`}
      style={{ transition: 'width 0.3s cubic-bezier(0.4, 0, 0.2, 1)' }}
    >
      {/* Header */}
      <div className={`${isDark ? 'border-gray-700' : 'border-gray-200'} border-b transition-all duration-300 h-[72px] flex items-center px-5`}>
        <div
        ...
        >
          ...
          >
            <h2 className="text-2xl font-extrabold bg-gradient-to-r from-slate-500 to-rose-400 bg-clip-text text-transparent whitespace-nowrap">
              Admin Panel
            </h2>
          </div>
        </div>
              ...
              <a
                ...
                className={`flex items-center p-3 rounded-xl transition-all duration-200 group relative ${
                  isActive(menu.path)
                    ...
                    : menu.highlight
                    ? `${isDark ? 'text-gray-300 hover:bg-gradient-to-r hover:from-[#DA7B93]/10 hover:to-[#2F4454]/10 hover:text-[#DA7B93]' : 'text-gray-700 hover:bg-gradient-to-r hover:from-[#DA7B93]/10 hover:to-[#2F4454]/10 hover:text-[#2F4454]'} border border-[#DA7B93]/20`
                    : `${isDark ? 'text-gray-300 hover:bg-slate-500/10 hover:text-rose-400' : 'text-gray-700 hover:bg-slate-500/5 hover:text-rose-500'}`
                ...
                <span
                  className={`font-medium whitespace-nowrap overflow-hidden transition-all duration-300 ${
                    ...
                    isActive(menu.path)
                      ...
                      : menu.highlight
                      ? `${isDark ? 'text-gray-300 group-hover:text-[#DA7B93]' : 'text-[#2F4454] group-hover:text-[#2F4454]'}`
                      : `${isDark ? 'text-gray-300 group-hover:text-rose-400' : 'text-gray-700 group-hover:text-rose-500'}`
                  }`}
                >
                  {menu.label}
                </span>
