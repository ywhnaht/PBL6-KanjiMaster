    ...
    if _model is None:
        with _model_lock:
            if _model is None:
                ...
                ckpt_path = "core/vit_checkpoint.pth"
...
if os.path.exists(token_file) and os.path.exists(ngram_file):
    ...
    print("⚠️ Không tìm thấy token/ngram pickle. Kiểm tra đường dẫn core/....")

# ==== Load JMdict (tiếng Anh) ====
jmdict_dir = "core/JMdict_english"
term_entries = []
if os.path.isdir(jmdict_dir):
    ...
    all_terms = set(entry[0] for entry in term_entries) if term_entries else set()
    print("✅ JMdict terms loaded:", len(all_terms))
else:
    all_terms = set()
    print("⚠️ JMdict directory not found:", jmdict_dir)

# ==== Hàm lọc / gợi ý từ ghép ====
...
# ==== EXAMPLE USAGE FUNCTIONS (print results) ====
def demo_from_strokes(strokes):
    img = strokes_to_image(strokes)
    merged = segment_characters_from_image(img, k=5)
    print("Merged boxes + predictions:")
